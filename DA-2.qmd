...
$$
y_{ijklm} = \mu + \tau_i + \alpha_j + \beta_k + (\tau \alpha)_{ij} + (\tau \beta)_{ik} + (\alpha \beta)_{jk} + (\tau \alpha \beta)_{ijk} + u_{l} +  v_{m(l)} + w_{n(l, m)} + \epsilon_{ijklm}
$$ 
